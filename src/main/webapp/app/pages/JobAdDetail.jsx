...
import { Header, Icon, Table, Button, Grid, Card, TableCell } from "semantic-ui-react";
      ...
      <Grid stackable>
        <Grid.Row>
          <Grid.Column width={6}>
            ...
            <br></br>
            {authItem[0].loggedIn && authItem[0].user.userType===1 && 
              <Button fluid color={favorites.includes(jobAd.id)?"blue":"green"} onClick={() => handleAddFavorites(jobAd.id)}>
                <Icon name={favorites.includes(jobAd.id)?"hand point right":"hand point right outline"} />{favorites.includes(jobAd.id)?"Başvurdunuz":"Başvur"}
              </Button>
            } 
